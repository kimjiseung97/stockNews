...
- 백엔드: Kotlin 2.3, Spring Boot 4.1, Local 환경MySQL(Aiven 원격 호스팅),배포 환경 MariaDb(홈서버 Docker 내부통신), Spring Batch 6, QueryDSL
...
- 데이터소스는 배포 서버(자체 홈서버, RAM 16GB) 안 Docker Compose로 같이 띄운 MariaDB 컨테이너(내부 네트워크 `db:3306`). 드라이버는 `org.mariadb.jdbc.Driver`(`mariadb-java-client`) 사용 — MySQL Connector/J는 MariaDB 대상으로 `INFORMATION_SCHEMA.KEYWORDS` 등 MySQL 8 전용 메타데이터 쿼리를 날려 Hibernate dialect 감지가 실패하므로 쓰지 말 것. `spring.jpa.hibernate.ddl-auto: update` — Spring Batch 메타데이터 테이블 제외하고 Hibernate 자동 스키마 관리.
...
- 배포 대상은 GCP가 아니라 **자체 홈서버(RAM 16GB)** — `.github/workflows/deploy.yml`이 SSH(포트 2222)로 접속해 `~/stockNews`에서 `git reset --hard origin/main` + `docker compose up -d --build`. 접속 정보(`SSH_HOST`/`SSH_USER`/`SSH_PASSWORD`)와 도메인(`DOMAIN`/`ADMIN_DOMAIN`)은 GitHub Actions 시크릿에만 두고 이 파일이나 저장소에 절대 적지 말 것.
- `Caddyfile`은 두 도메인을 프록시한다: `{$DOMAIN}` → `app:8080`(본 서비스), `{$ADMIN_DOMAIN}` → `stocknewsadmin-app-1:8081`(별도 compose 프로젝트인 어드민, 같은 `stocknews_default` 네트워크).
